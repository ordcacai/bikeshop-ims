...
if(!isset($_SESSION['logged_in'])){
    ...
    $stmt = $conn->prepare("SELECT * FROM stock_transfer WHERE transfer_type = 'Outbound' AND DATE(transfer_date) = curdate()");
    $stmt->execute();
...
$pdf->Cell(25	,5,'Recipient',1,1,'C');//end of line
...
while($row = $stocks->fetch_assoc()){
    ...
    $pdf->Cell(25	,5,$row['location_to'],1,1,'C');
    //end of line
}
